...
VEXYL_TTS_MAX_ACTIVE_GENERATIONS=1
VEXYL_TTS_MAX_QUEUE_SIZE=100
VEXYL_TTS_STREAM_PLAY_STEPS=40
VEXYL_TTS_STREAMER_TIMEOUT=30
...
| `VEXYL_TTS_MAX_ACTIVE_GENERATIONS` | `1` | Any integer | Max active model generations. Keep low until measured on target hardware |
| `VEXYL_TTS_MAX_QUEUE_SIZE` | `100` | Any integer | Max queued streaming chunks/segments per request |
| `VEXYL_TTS_STREAM_PLAY_STEPS` | `40` | `10`-`120` | Default Parler streaming interval. Lower = faster first audio, higher = less decoder overhead |
| `VEXYL_TTS_STREAMER_TIMEOUT` | `30` | Seconds | Max wait for a streamer chunk before returning an error |
...
Client                              Server
  ...
  |<---- {"type":"audio",...} ---------|  (non-streaming response)
  |<---- {"type":"audio_chunk",...} ---|  (streaming chunk)
  |<---- {"type":"audio_end",...} -----|  (streaming complete)
...
| `{"type":"synthesize","text":"...","stream":true,"include_full_audio":false,"play_steps":40}` | Stream model audio chunks as they are generated. `include_full_audio` defaults to `false` |
| `{"type":"stream_text_start","request_id":"x","lang":"hi-IN","style":"warm"}` | Start a RAG-style text stream |
| `{"type":"stream_text_delta","request_id":"x","text":"..."}` | Add partial RAG text. The server buffers until a sentence/clause boundary |
| `{"type":"stream_text_end","request_id":"x"}` | Flush remaining RAG text and finish the stream |
| `{"type":"cancel","request_id":"x"}` | Stop queued RAG segments and suppress further audio for the request |
...
| `{"type":"audio_chunk","request_id":"x","chunk_index":0,"audio_b64":"...","sample_rate":22050,"cached":false}` | Streaming WAV chunk |
| `{"type":"audio_end","request_id":"x","total_chunks":4,"first_chunk_ms":650,"latency_ms":2400,"sample_rate":22050}` | Streaming complete. Includes `full_audio_b64` only when requested |
| `{"type":"stream_text_ready","request_id":"x","play_steps":40}` | RAG text stream accepted |
| `{"type":"stream_text_done","request_id":"x"}` | RAG text stream flushed |
| `{"type":"cancelled","request_id":"x"}` | Cancellation acknowledged |

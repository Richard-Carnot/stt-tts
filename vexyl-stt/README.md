...
<p align="center">
  ...
  <a href="#supported-languages"><img src="https://img.shields.io/badge/Languages-23-orange.svg" alt="Languages"></a>
</p>

WebSocket + REST speech-to-text server wrapping the [ai4bharat/indic-conformer-600m-multilingual](https://huggingface.co/ai4bharat/indic-conformer-600m-multilingual) model (600M parameters) for 22 Indian languages, plus [Distil-Whisper](https://huggingface.co/distil-whisper/distil-medium.en) for English ASR. Self-hosted, zero API costs, full data sovereignty.
...
- 22 Indian languages + English supported
- **Automatic Language Detection (LID)** — Send `"lang": "auto"` to automatically classify spoken language (Vakgyata) and route to either Distil-Whisper or Indic Conformer on a per-utterance basis
...
| `VEXYL_STT_EN_ENABLED` | `true` | `true`, `false` | Load Distil-Whisper for English ASR. Set `false` to save memory on Indic-only deployments |
| `VEXYL_STT_EN_MODEL` | `distil-whisper/distil-medium.en` | HuggingFace model ID | English ASR model. Swap to `distil-whisper/distil-large-v3` for higher accuracy |
| `VEXYL_STT_LID_ENABLED` | `true` | `true`, `false` | Load Vakgyata for spoken language identification (LID) and automatic routing |
| `VEXYL_STT_LID_MODEL` | `onecxi/vakgyata-base` | HuggingFace model ID | Spoken LID model. Swap to `onecxi/vakgyata-tiny` if memory/CPU is restricted |
| `VEXYL_STT_LID_MIN_CONFIDENCE` | `0.3` | Float `0.0 - 1.0` | Overall confidence threshold to trust any LID classification. Otherwise falls back to session default language |
| `VEXYL_STT_LID_EN_THRESHOLD` | `0.85` | Float `0.0 - 1.0` | High-confidence English threshold specifically for routing to Distil-Whisper |
| `VEXYL_STT_LID_SESSION_LOCK` | `true` | `true`, `false` | Lock language resolved for the session after the first utterance to prevent phonetic flickering |
